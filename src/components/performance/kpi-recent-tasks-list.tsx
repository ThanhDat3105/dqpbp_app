    ...
    <View className="mx-4 mb-3 rounded-xl border border-gray-100 bg-white shadow-sm overflow-hidden">
      ...
      {loading ? (
        <View className="p-4" style={{ gap: 10 }}>
          {[1, 2, 3].map((i) => <SkeletonBox key={i} height={56} />)}
        </View>
      ) : tasks.length === 0 ? (
        ...
        tasks.map((task) => {
          ...
          return (
            <View key={task.id} className="flex-row items-center gap-3 px-4 py-3 border-b border-gray-50">
              <View className="flex-1">
                <ThemedText style={{ fontSize: 13, fontWeight: "600", color: "#111827" }} numberOfLines={1}>
                  {task.title}
                </ThemedText>
